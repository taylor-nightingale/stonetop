# Translation worklist — de

Generated by `npm run i18n:handoff`. Every entry below is still in the authoring files with
your German attached — edit them there, not here.

- **34** entries where the English changed under an existing translation (4 of them markup only)
- **35** translations whose row was restructured and need re-filing

## playbooks

### the-judge

#### `backgrounds/missionary/description`

This row was restructured; the key is gone. Your German:

```
Du gehörst einem größeren Orden von Richtern an, die hierher entsandt wurden, um die flackernde Flamme der Zivilisation zu schützen. Die Chronik ist noch relativ neu; deine Stellung im Dorf ist alles andere als sicher. Füge diese Richter dem Abschnitt „Nachbarn“ im Stonetop-Spielbuch hinzu (wähle 2 weitere):

Wenn du **_den Richter einer anderen Siedlung um Hilfe oder Informationen bittest_**, ist dieser durch einen Eid verpflichtet, sie dir zu gewähren. Ebenso bist du durch einen Eid verpflichtet, ihn zu unterstützen.

Zusätzlich zu deiner üblichen Auswahl an besonderen Besitztümern verfügst du über ein Vogelhaus (markiere es jetzt). Wenn du, wie es bei den Richtern deines Ordens üblich ist, **_eine Nachricht mit einem abgerichteten Vogel verschickst_**, teilt dir der SL mit, ob und wann du eine Antwort erhältst und was darin steht.
```

Untranslated keys left in this document — the slots these words belong in:

- `backgrounds/missionary/choices/devin/text` — "Devin (from Marshedge)"
- `backgrounds/missionary/choices/haeris/text` — "Haeris (from Gordin's Delve)"
- `backgrounds/missionary/choices/isalde/text` — "Isalde (from the Manmarch)"
- `backgrounds/missionary/choices/Rahat/text` — "Rahat (from Lygos)"
- `backgrounds/missionary/choices/tejisha/text` — "Tejisha (from Barrier Pass)"
- `backgrounds/missionary/choices/unz/text` — "Unz (from the Hillfolk)"

#### `backgrounds/prophet/description`

The English changed after this was translated.

Was:

```
The line of Judges was broken long ago, the Chronicle lost or fallen into ruin. Aratis has called you personally to her service through dreams, omens, and visions. Some in town resent the authority you've assumed.

When you **_spend a few days communing with Aratis about a threat facing Stonetop or civilization as a whole_**, roll +WIS: **on a 7+**, Aratis reveals the course of action she would have you take; **on a 10+**, you also hold 2 Sanction. While **_acting on her orders_**, spend 1 Sanction to add +1 to a roll you just made.
```

Now:

```
The line of Judges was broken long ago, the Chronicle lost or fallen into ruin. Aratis has called you personally to her service through dreams, omens, and visions. Some in town resent the authority you've assumed.
```

Your German:

```
Die Linie der Richter ist vor langer Zeit abgerissen, die Chronik verloren gegangen oder dem Verfall anheimgefallen. Aratis hat dich persönlich durch Träume, Omen und Visionen in ihren Dienst berufen. Einige im Ort nehmen dir die Autorität übel, die du dir anmaßt.

Wenn du **_einige Tage damit verbringst, mit Aratis über eine Bedrohung für Stonetop oder die Zivilisation als Ganzes Zwiesprache zu halten_**, würfle +WE. Bei einer 7+ offenbart dir Aratis die Vorgehensweise, die sie von dir erwartet. Bei einer 10+ erhältst du außerdem 2 Gunst. Während du nach ihren Anweisungen handelst, kannst du 1 Gunst ausgeben, um +1 auf einen gerade abgelegten Wurf zu erhalten.
```

#### `backgrounds/missionary/choices/0/text`

Markup or spacing only — the wording did not change.

Was:

```
You are part of a larger order of Judges, sent here to protect the flickering flame of civilization. The Chronicle is relatively new; your position in town is far from certain. Add these Judges to the Neighbors section of the steading playbook (pick 2 more):
```

Now:

```
You are part of a larger order of Judges, sent here to protect the flickering flame of civilization. The Chronicle is relatively new; your position in town is far from certain. Add these Judges to the Neighbors section of the steading playbook (pick 2 more):
```

Your German:

```
Du gehörst einem größeren Orden von Richtern an, die hierher entsandt wurden, um die flackernde Flamme der Zivilisation zu schützen. Die Chronik ist noch relativ neu; deine Stellung im Dorf ist alles andere als sicher. Füge diese Richter dem Abschnitt „Nachbarn“ im Stonetop-Spielbuch hinzu (wähle 2 weitere):
```

### the-lightbearer

#### `choices/helior-worship-spread/0/title`

The English changed after this was translated.

Was:

```
The Worship of Helior Is…
```

Now:

```
Praise the day
```

Your German:

```
Die Verehrung Heliors ist...
```

#### `choices/helior-worship-spread/0/text`

The English changed after this was translated.

Was:

```
You are the appointed servant of Helior the Daybringer, god of the sun and light, beacon of hope and mercy. *(choose 1)*
```

Now:

```
You are the appointed servant of Helior the Daybringer, god of the sun and light, beacon of hope and mercy.
```

Your German:

```
Du bist der auserwählte Diener Heliors, dem Tagesbringer, dem Gott der Sonne und des Lichts, dem Leuchtfeuers der Hoffnung und Barmherzigkeit.
```

#### `choices/helior-worship-practice/0/text`

The English changed after this was translated.

Was:

```
*(choose 1 or 2)*
```

Now:

```
He Is Worshipped Through… (choose 1 or 2)
```

Your German:

```
(wähle 1 oder 2)
```

#### `choices/helior-worship-practice/0/title`

This row was restructured; the key is gone. Your German:

```
Er wird verehrt durch...
```

Untranslated keys left in this document — the slots these words belong in:

- `choices/helior-worship-spread/1/text` — "The Worship of Helior Is… (choose 1)"

#### `choices/helior-shrine/0/text`

The English changed after this was translated.

Was:

```
*(choose 1)*
```

Now:

```
In Stonetop's Pavilion of the Gods, Helior's Shrine Has… (choose 1)
```

Your German:

```
(wähle 1)
```

#### `choices/helior-shrine/0/title`

This row was restructured; the key is gone. Your German:

```
Heliors Schrein im Götterpavillon von Stonetop...
```

Untranslated keys left in this document — the slots these words belong in:

- `choices/helior-worship-spread/1/text` — "The Worship of Helior Is… (choose 1)"

#### `choices/helior-predecessor/0/text`

The English changed after this was translated.

Was:

```
*(choose 2 or 3)*
```

Now:

```
Your Predecessor, the Previous Lightbearer… (choose 2 or 3)
```

Your German:

```
(wähle 2 oder 3)
```

#### `choices/helior-predecessor/0/title`

This row was restructured; the key is gone. Your German:

```
Dein Vorgänger, der vorherige Lichtbringer...
```

Untranslated keys left in this document — the slots these words belong in:

- `choices/helior-worship-spread/1/text` — "The Worship of Helior Is… (choose 1)"

#### `choices/helior-powers-origin/0/text`

The English changed after this was translated.

Was:

```
*(choose 1)*
```

Now:

```
You Came Into Your Powers… (choose 1)
```

Your German:

```
(wähle 1)
```

#### `choices/helior-powers-origin/0/title`

This row was restructured; the key is gone. Your German:

```
Du erlangtest deine Kräfte…
```

Untranslated keys left in this document — the slots these words belong in:

- `choices/helior-worship-spread/1/text` — "The Worship of Helior Is… (choose 1)"

### the-marshal

#### `choices/war-stories-action/0/title`

The English changed after this was translated.

Was:

```
The Last Time the Militia Saw Serious Action, It Was…
```

Now:

```
War stories
```

Your German:

```
The last time the militia saw serious action, it was...
```

#### `choices/war-stories-action/0/text`

The English changed after this was translated.

Was:

```
*(pick 1)*
```

Now:

```
The Last Time the Militia Saw Serious Action, It Was… (pick 1)
```

Your German:

```
wähle 1
```

## moves

### amulets-talismans

#### `description`

The English changed after this was translated.

Was:

```
When you **_craft a protective charm for someone_**, spend 1 Stock and name a source of harm (fire, stabbing, etc.). When **_they would suffer such harm while bearing your charm_**, roll +INT: **on a 10+**, they ignore the harm entirely; **_on a 7-9_**, they suffer only half the damage or effect; **on a 6-**, they suffer the harm normally. One can benefit from only 1 charm at a time, and it loses its potency after 1 use.
```

Now:

```
When you **_craft a protective charm for someone_**, spend 1 Stock and name a source of harm (fire, stabbing, etc.). When **_they would suffer such harm while bearing your charm_**, roll +INT: **on a 10+**, they ignore the harm entirely; **on a 7-9**, they suffer only half the damage or effect; **on a 6-**, they suffer the harm normally. One can benefit from only 1 charm at a time, and it loses its potency after 1 use.
```

Your German:

```
Wenn du **_für jemanden einen Schutzzauber anfertigst_**, gib 1 Vorrat aus und nenne eine Schadensquelle (Feuer, Stichverletzung usw.). Wenn die Person, **_die deinen Zauber trägt, solchen Schaden erleiden würde_**, würfle +IN. **Bei einer 10+** ignoriert sie den Schaden vollständig. **Bei 7–9** erleidet sie nur den halben Schaden oder Effekt. **Bei einer 6−** erleidet sie den Schaden wie üblich. Eine Person kann immer nur von 1 Zauber gleichzeitig profitieren, und nach einer Anwendung verliert der Zauber seine Wirkung.
```

### bolster

#### `description`

The English changed after this was translated.

Was:

```
When you **_prepare for what's coming or seek the favor of the gods_**, say how and answer the GM's questions. Then, hold Preperation based on the amount of time you devote:

- A week or so: 1 Preperation
- A month or so: 2 Preperation
- The better part of a season: 3 Preperation
 When you **_make any roll to which your efforts might apply_**, you may spend 1 Preperation to add +1 to that roll, after it is made (maximum +1 per roll).
```

Now:

```
When you **_prepare for what's coming or seek the favor of the gods_**, say how and answer the GM's questions. Then, hold Preparation based on the amount of time you devote:

- A week or so: 1 Preparation
- A month or so: 2 Preparation
- The better part of a season: 3 Preparation

When you **_make any roll to which your efforts might apply_**, you may spend 1 Preparation to add +1 to that roll, after it is made (maximum +1 per roll).
```

Your German:

```
Wenn du **_dich auf das, was dir bevorsteht, vorbereitest oder die Gunst der Götter suchst_**, beschreibe, wie du dabei vorgehst, und beantworte die Fragen des SL. Nimm dann entsprechend der Zeit, die du dafür aufgewendet hast, Vorbereitung:

- Etwa eine Woche: 1 Vorbereitung
- Etwa einen Monat: 2 Vorbereitung
- Den größten Teil einer Jahreszeit: 3 Vorbereitung
 Wenn du **_würfelst und deine Vorbereitung Einfluss auf das Ergebnis haben könnte_**, kannst du nach dem Würfeln 1 Vorbereitung ausgeben, um +1 auf das Ergebnis zu erhalten (maximal +1 je Wurf).
```

### danus-grasp

#### `description`

The English changed after this was translated.

Was:

```
When you **_call on the world itself to bind a spirit or a perversion of nature_**, spend 1 Stock and roll +WIS: **_on a 7+_**, roots, vines, and earth pull at them, and they pick 1; **on a 10+**, as a 7-9, but both apply.

- They're restrained, unable to act freely until your focus slips or they tear their way free
- They take 2d4 damage (ignores armor)
If this brings them to 0 HP, they are pulled into the earth and bound in rune-etched stone.
```

Now:

```
When you **_call on the world itself to bind a spirit or a perversion of nature_**, spend 1 Stock and roll +WIS: **on a 7+**, roots, vines, and earth pull at them, and they pick 1; **on a 10+**, as a 7-9, but both apply.

- They're restrained, unable to act freely until your focus slips or they tear their way free
- They take 2d4 damage (ignores armor)
If this brings them to 0 HP, they are pulled into the earth and bound in rune-etched stone.
```

Your German:

```
Wenn du **_die Welt selbst anrufst, um einen Geist oder eine Perversion der Natur zu binden_**, gib 1 Vorrat aus und würfle +WE. **Bei einer 7+** greifen Wurzeln, Ranken und Erde nach ihnen, und sie wählen 1. **Bei einer 10+**, wie bei 7–9, aber beide Effekte treten ein.

- Sie sind gefesselt und können nicht frei handeln, bis deine Konzentration nachlässt oder sie sich losreißen.
- Sie erleiden 2W4 Schaden (ignoriert Rüstung).
Wenn ihre TP dadurch auf 0 sinken, werden sie in die Erde gezogen und in einen mit Runen verzierten Stein verwandelt.
```

### seasons-change-autumn

#### `description`

The English changed after this was translated.

Was:

```
When **_autumn falls and the land is ripe for harvest_**, whoever is most determined rolls +Fortunes: **on a 10+**, pick 1 seasonal gain; **on a 7-9**, pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse; **on a 6-**, threats abound (and don't mark XP).

Whatever the result, reset Fortunes to +1.

When the harvest is complete, roll 1d4; the steading generates that much Surplus.

**Seasonal gains** (pick from these when the move tells you to):

- **Population boom:** A number of youth come of age, and/or outsiders settle here. Increase Population by 1 (max +3).
- **Tor's blessing:** Fine weather abounds. Take +1 to Pull Together this season, and any time you roll the Die of Fate for weather, roll twice and take your pick.
- **Unexpected bounty:** A sudden influx of wild game, trade profits, or some other resource generates 1 Surplus, now.
- **Trade opportunity:** At some point this season, someone offers to trade something valuable at a good price or something unique/unusual at a reasonable price. Pay what they're asking and it's yours.
- **Interesting news:** There's an opportunity to improve your fortunes, knowledge, or relations, and/or to make progress towards a steading improvement.
- **Valuable insight:** You learn something that gives you a chance to address a threat that's been plaguing the steading.
```

Now:

```
When **_autumn falls and the land is ripe for harvest_**, whoever is most determined rolls +Fortunes: **on a 10+**, pick 1 seasonal gain; **on a 7-9**, pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse; **on a 6-**, threats abound (and don't mark XP).

Whatever the result, reset Fortunes to +1.

When **_the harvest is complete_**, roll 1d4; the steading generates that much Surplus.

**Seasonal gains** (pick from these when the move tells you to):

- **Population boom:** A number of youth come of age, and/or outsiders settle here. Increase Population by 1 (max +3).
- **Tor's blessing:** Fine weather abounds. Take +1 to Pull Together this season, and any time you roll the Die of Fate for weather, roll twice and take your pick.
- **Unexpected bounty:** A sudden influx of wild game, trade profits, or some other resource generates 1 Surplus, now.
- **Trade opportunity:** At some point this season, someone offers to trade something valuable at a good price or something unique/unusual at a reasonable price. Pay what they're asking and it's yours.
- **Interesting news:** There's an opportunity to improve your fortunes, knowledge, or relations, and/or to make progress towards a steading improvement.
- **Valuable insight:** You learn something that gives you a chance to address a threat that's been plaguing the steading.
```

Your German:

```
Wenn **_der Herbst hereinbricht und das Land reif für die Ernte ist_**, würfelt derjenige, der am entschlossensten ist, 2W6+Schicksal. **Bei einer 10+** wähle 1 saisonalen Gewinn. **Bei 7–9** wähle 1 saisonalen Gewinn, aber eine Bedrohung für die Siedlung zeichnet sich ab oder verschlimmert sich. **Bei einer 6-** nehmen die Bedrohungen überhand (markiere keine EP).

Unabhängig vom Ergebnis wird Schicksal auf +1 zurückgesetzt.

Wenn **_die Ernte abgeschlossen ist_**, würfle 1W4. Die Siedlung erzeugt diese Menge an Überschuss.
```

#### `moveResults/success/value`

The English changed after this was translated.

Was:

```
Pick 1 seasonal gain.

Then reset Fortunes to +1. When the harvest is complete, roll 1d4; the steading generates that much Surplus.
```

Now:

```
Pick 1 seasonal gain.
```

Your German:

```
Wähle 1 saisonalen Gewinn.

Setze dann Schicksal auf +1 zurück. Wenn die Ernte abgeschlossen ist, würfle 1W4. Die Siedlung erzeugt diese Menge an Überschuss.
```

#### `moveResults/partial/value`

The English changed after this was translated.

Was:

```
Pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse.

Then reset Fortunes to +1. When the harvest is complete, roll 1d4; the steading generates that much Surplus.
```

Now:

```
Pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse.
```

Your German:

```
Wähle 1 saisonalen Gewinn, aber eine Bedrohung für die Siedlung zeichnet sich ab oder verschlimmert sich.

Setze dann Schicksal auf +1 zurück. Wenn die Ernte abgeschlossen ist, würfle 1W4. Die Siedlung erzeugt diese Menge an Überschuss.
```

#### `moveResults/failure/value`

The English changed after this was translated.

Was:

```
Threats abound (and don't mark XP).

Then reset Fortunes to +1. When the harvest is complete, roll 1d4; the steading generates that much Surplus.
```

Now:

```
Threats abound (and don't mark XP).
```

Your German:

```
Die Bedrohungen nehmen überhand (markiere keine EP).

Setze dann Schicksal auf +1 zurück. Wenn die Ernte abgeschlossen ist, würfle 1W4. Die Siedlung erzeugt diese Menge an Überschuss.
```

### seasons-change-spring

#### `moveResults/success/value`

The English changed after this was translated.

Was:

```
Pick 1 seasonal gain.

Then reset Fortunes to +1.
```

Now:

```
Pick 1 seasonal gain.
```

Your German:

```
Wähle 1 saisonalen Gewinn.

Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/partial/value`

The English changed after this was translated.

Was:

```
Pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse.

Then reset Fortunes to +1.
```

Now:

```
Pick 1 seasonal gain, but a threat to the steading makes itself known or gets worse.
```

Your German:

```
Wähle 1 saisonalen Gewinn, aber eine Bedrohung für die Siedlung zeichnet sich ab oder verschlimmert sich.

Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/failure/value`

The English changed after this was translated.

Was:

```
Threats abound (and don't mark XP).

Then reset Fortunes to +1.
```

Now:

```
Threats abound (and don't mark XP).
```

Your German:

```
Die Bedrohungen nehmen überhand (markiere keine EP).

Setze dann Schicksal auf +1 zurück.
```

### seasons-change-summer

#### `moveResults/success/value`

The English changed after this was translated.

Was:

```
Pick 2 seasonal gains.

The steading generates 1d4-1 Surplus. Then reset Fortunes to +1.
```

Now:

```
Pick 2 seasonal gains.
```

Your German:

```
Wähle 2 saisonalen Gewinne.

Die Siedlung erzeugt  1W4−1 Überschuss. Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/partial/value`

The English changed after this was translated.

Was:

```
Pick 1 seasonal gain.

The steading generates 1d4-1 Surplus. Then reset Fortunes to +1.
```

Now:

```
Pick 1 seasonal gain.
```

Your German:

```
Wähle 1 saisonalen Gewinn.

Die Siedlung erzeugt  1W4−1 Überschuss. Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/failure/value`

The English changed after this was translated.

Was:

```
A threat to the steading makes itself known or gets worse (and don't mark XP).

The steading generates 1d4-1 Surplus. Then reset Fortunes to +1.
```

Now:

```
A threat to the steading makes itself known or gets worse (and don't mark XP).
```

Your German:

```
Eine Bedrohung für die Siedlung zeichnet sich ab oder verschlimmert sich.

Die Siedlung erzeugt  1W4−1 Überschuss. Setze dann Schicksal auf +1 zurück.
```

### seasons-change-winter

#### `description`

The English changed after this was translated.

Was:

```
When **_winter grips the land_**, whoever is weariest rolls 1d4+Population; the steading consumes that much Surplus. If there's not enough, reduce Surplus to 0 and Meet with Disaster. Then, pick 1:

- Reduce Population by 1 (min -1) due to death, decrepitude, and departure
- An important resource (one of the horses, the cistern, etc.) is lost or not maintained
- An important NPC dies, their role unfilled
- Your PC dies, leaves, or retires from play

Then, roll +Fortunes: **on a 10+**, the winter is relatively mild, and each player can name a local NPC with whom their relationship improves at least a little (work together to figure out how); **on a 7-9**, the steading must consume additional Surplus equal to 1d4+Population before winter ends or suffer the consequences as above; **on a 6-**, as a 7-9, but also threats abound (and don't mark XP).

Whatever the result, reset Fortunes to +1.
```

Now:

```
When **_winter grips the land_**, whoever is weariest rolls 1d4+Population; the steading consumes that much Surplus. If there's not enough, reduce Surplus to 0 and @UUID[Compendium.stonetop.moves.Item.4TYC5OYdPbAxWHQp]{Meet with Disaster}. Then, pick 1:

- Reduce Population by 1 (min -1) due to death, decrepitude, and departure
- An important resource (one of the horses, the cistern, etc.) is lost or not maintained
- An important NPC dies, their role unfilled
- Your PC dies, leaves, or retires from play

Then, roll +Fortunes: **on a 10+**, the winter is relatively mild, and each player can name a local NPC with whom their relationship improves at least a little (work together to figure out how); **on a 7-9**, the steading must consume additional Surplus equal to 1d4+Population before winter ends or suffer the consequences as above; **on a 6-**, as a 7-9, but also threats abound (and don't mark XP).

Whatever the result, reset Fortunes to +1.
```

Your German:

```
Wenn **_der Winter das Land fest im Griff hat_**, würfelt derjenige, der am erschöpftesten ist, 1W4+Bevölkerung (Minimum 0). Die Siedlung verbraucht so viel Überschuss. Wenn nicht genügend Überschuss vorhanden ist, setze Überschuss auf 0, senke Schicksal um 1 und wähle 1:

- Reduziere die Bevölkerung um 1 (Minimum −1) aufgrund von Tod, Gebrechlichkeit und Abwanderung.
- Eine wichtige Ressource (eines der Pferde, die Zisterne usw.) geht verloren oder wird nicht instand gehalten.
- Ein wichtiger NSC stirbt; seine Position bleibt unbesetzt.
- Dein SC stirbt, geht fort oder zieht sich aus dem Spiel zurück.
Würfle anschließend 2W6+Schicksal. **Bei einer 10+** ist der Winter vergleichsweise mild, und jeder Spieler darf einen lokalen NSC nennen, zu dem sich seine Beziehung zumindest ein wenig verbessert. Überlegt gemeinsam, wie es dazu kommt. **Bei 7–9** muss die Siedlung vor Ende des Winters zusätzlichen Überschuss in Höhe von 1W4+Bevölkerung verbrauchen. Andernfalls treten die oben genannten Konsequenzen ein. **Bei einer 6-** gilt dasselbe wie bei 7–9, aber zusätzlich nehmen die Bedrohungen überhand (markiere keine EP).

Unabhängig vom Ergebnis wird Schicksal auf +1 zurückgesetzt.
```

#### `moveResults/success/value`

The English changed after this was translated.

Was:

```
The winter is relatively mild, and each player can name a local NPC with whom their relationship improves at least a little (work together to figure out how).

Then reset Fortunes to +1.
```

Now:

```
The winter is relatively mild, and each player can name a local NPC with whom their relationship improves at least a little (work together to figure out how).
```

Your German:

```
Der Winter ist vergleichsweise mild, und jeder Spieler darf einen lokalen NSC nennen, zu dem sich seine Beziehung zumindest ein wenig verbessert. Überlegt gemeinsam, wie es dazu kommt.

Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/partial/value`

The English changed after this was translated.

Was:

```
The steading must consume additional Surplus equal to 1d4+Population before winter ends or suffer the consequences as above.

Then reset Fortunes to +1.
```

Now:

```
The steading must consume additional Surplus equal to 1d4+Population before winter ends or suffer the consequences as above.
```

Your German:

```
Die Siedlung muss vor Ende des Winters zusätzlichen Überschuss in Höhe von 1W4+Bevölkerung verbrauchen. Andernfalls treten die oben genannten Konsequenzen ein.

Setze dann Schicksal auf +1 zurück.
```

#### `moveResults/failure/value`

The English changed after this was translated.

Was:

```
As a 7-9, but also threats abound (and don't mark XP).

Then reset Fortunes to +1.
```

Now:

```
As a 7-9, but also threats abound (and don't mark XP).
```

Your German:

```
Wie bei 7-9 aber die Bedrohungen nehmen überhand (markiere keine EP).n
Setze dann Schicksal auf +1 zurück.
```

## arcana

### cracked-flute

#### `front/choices/cracked-flute/1/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_einige Tage lang übst und dabei die Flöte dort spielst, wo der Wind dich hören kann_**, würfle +CH. **Bei einer 10+** markiere 1 Kreis. **Bei 7–9** markiere 1 Kreis, aber du beschwörst einen zornigen Zephyrgeist. Bei einer **6−** machst du keine Fortschritte, schaffst es aber dennoch, einen zornigen Zephyrgeist zu beschwören.
```

Untranslated keys left in this document — the slots these words belong in:

- `front/choices/cracked-flute/3/text` — "When you **_mark all three circles_**, you unlock the mysteries of the flute."

### staff-of-the-lidless-orb

#### `front/choices/staff-of-the-lidless-orb/1/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_den Stab umfasst, die Augen schließt und durch die lidlose Kugel siehst_**, würfle +KO. **Bei einer 10+** wähle 3 aus der folgenden Liste. **Bei 7–9** wähle 1.


- Die Kugel sieht durch natürliche oder magische Dunkelheit hindurch.
- Die Kugel sieht durch Illusionen, Trugbilder und Unsichtbarkeit hindurch.
- Die Kugel kann starke Emotionen als Auren erkennen, die von Menschen ausstrahlen und an Orten verweilen.
- Du kannst dich bewegen und effektiv handeln, während du durch die Kugel siehst (andernfalls eher nicht).

Was auch immer du wählst, der Effekt hält an, bis du die Augen öffnest oder den Stab loslässt.

Bei einer 6− zeigt dir die Kugel eine schreckliche Vision von einem fernen Ort oder einer fernen Zeit. Markiere 1 (oder siehe unten) und frage den SL, was du siehst. Solange die Vision andauert, nimmst du die Welt um dich herum nicht wahr und du hast bei deinem ersten Wurf, nachdem du wieder zu dir gekommen bist, einen Nachteil.
```

No untranslated keys remain in this document — this text may simply be gone.

### storm-markings

#### `front/choices/storm-markings/1/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_vor Wut kochst_**, verursachst du +1 Schaden, bis du dich wieder beruhigt hast. Wenn du **_jedoch versuchst, dein Temperament zu zügeln_**, würfle +WE. **Bei einer 10+** behältst du einen kühlen Kopf und handelst, wie du willst. **Bei 7–9** wählst du 1 aus der folgenden Liste. **Bei einer 6-** drehst du einfach durch – sag dem SL, was für eine Dummheiten du am Ende anstellst.
- Atme einige Male tief durch und zähle bis zehn, während du die ganze Zeit vor Wut kochst.
- Lass deiner Wut freien Lauf, aber erzähle uns, wie und worüber.
```

Untranslated keys left in this document — the slots these words belong in:

- `back/choices/consequences/markings-c3/text` — "normally use that move). You can’t dismiss it easily; the effects (including the disadvantage) continue until you calm down."

### vein-of-milky-crystal

#### `front/choices/vein-of-milky-crystal/personally-extract/text`

Markup or spacing only — the wording did not change.

Was:

```
… personally extract a large hunk of crystal (Value 1) from the vein.
```

Now:

```
… personally extract a large ◇ hunk of crystal (Value 1) from the vein.
```

Your German:

```
… persönlich ein großes Stück Kristall (Wert 1) aus der Ader herausbrechen.
```

## possessions

### a-pot-of-gold

#### `description`

Markup or spacing only — the wording did not change.

Was:

```
◇◇ *magical*

A small bronze cauldron, full of glittering coins. At least, that’s what it seems.

When you **_fill the pot with small, natural_** **_items (leaves, stones, acorns, etc.) and hide_** **_it from light for a season_**, the contents are glamoured to become a purse of gold ◇ (Value 4).

Any “coins” removed from the pot revert to their true form when…

… an honest merchant touches them;<br>… their current owner tells a lie;<br>… someone sees through the glamour;<br>… something dispels the glamour; or<br>… the new moon comes and goes

*An artifact of the wider world — see @UUID[Compendium.stonetop.wider-world-and-other-wonders.JournalEntry.YMV8KIodnoZL71Jl]{Fae}.*
```

Now:

```
◇◇ *magical*

A small bronze cauldron, full of glittering coins. At least, that’s what it seems.

When you **_fill the pot with small, natural_** **_items (leaves, stones, acorns, etc.) and hide_** **_it from light for a season_**, the contents are glamoured to become a ◇ purse of gold (Value 4).

Any “coins” removed from the pot revert to their true form when…

… an honest merchant touches them;<br>… their current owner tells a lie;<br>… someone sees through the glamour;<br>… something dispels the glamour; or<br>… the new moon comes and goes

*An artifact of the wider world — see @UUID[Compendium.stonetop.wider-world-and-other-wonders.JournalEntry.YMV8KIodnoZL71Jl]{Fae}.*
```

Your German:

```
◇◇, *Magisch*

Ein kleiner Bronzekessel, gefüllt mit glitzernden Münzen. Zumindest scheint es so.

Wenn du **_den Topf mit kleinen natürlichen Dingen (Blätter, Steine, Eicheln usw.) füllst und ihn eine Jahreszeit lang vor Licht geschützt aufbewahrst_**, wird sein Inhalt verzaubert und verwandelt sich in einen Beutel voller Gold (Wert 4).

Alle „Münzen“, die aus dem Topf entfernt werden, verwandeln sich wieder in ihre wahre Form, wenn …

… ein ehrlicher Händler sie berührt; … ihr aktueller Besitzer lügt;<br>… jemand die Illusion durchschaut;<br>… etwas den Zauber aufhebt; oder<br>… der Neumond kommt und vergeht.

*Ein Artefakt aus der weiten Welt — siehe @UUID[Compendium.stonetop.wider-world-and-other-wonders.JournalEntry.YMV8KIodnoZL71Jl]{Feen}.*
```

## followers

### tulpa

#### `choices/choices/carry-manipulate-a-item/text`

Markup or spacing only — the wording did not change.

Was:

```
Carry/manipulate a item
```

Now:

```
Carry/manipulate a ◇ item
```

Your German:

```
Trage/manipuliere einen ◇ Gegenstand.
```

## steading-improvements

### additional-housing

#### `choices/4/text`

The English changed after this was translated.

Was:

```
And then: Pulling together 5 times, each requiring 1 season, 1 Surplus, and a wagonload of timber and other supplies (Value 2), to (re)build homes.
```

Now:

```
And then:
```

Your German:

```
Und anschließend: 5-mal gemeinsam anpacken, wobei jedes Mal 1 Jahreszeit, 1 Überschuss und eine Wagenladung Bauholz und anderer Materialien (Wert 2) benötigt werden, um Häuser zu bauen oder umzubauen.
```

#### `choices/5/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Schicksal um 1 und zeichne alle neuen Häuser auf der Karte ein.

Von nun an gilt: Wenn du **_im Winter Überschuss verbrauchst_**, setze die Bevölkerung um 1 niedriger an, als sie tatsächlich ist.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add any new homes to the map"
- `effects/2/text` — "consider Population to be 1 lower than it is"
- `effects/2/when/phrase` — "when you **_consume Surplus in winter_**"
- `choices/pull-together/text` — "Pulling together 5 times, each requiring 1 season, 1 Surplus, and a wagonload of timber and other supplies (Value 2), to (re)build homes."

### aurochs-hunting

#### `choices/7/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, füge „Auerochsenjagd (Fleisch, Fell, Horn)“ zur Ressourcenliste hinzu.

Von nun an gilt: Wenn du **_im Frühling die Auerochsenjagd anführst_**, würfle +Verteidigung. **Bei einer 10+** erhältst du 1W4 Überschuss. **Bei 7–9** erhältst du 1W4 Überschuss, aber wähle eines aus der folgenden Liste. **Bei einer 6-** wählst du eines aus der Liste – oder zwei und erhältst 1W4 Überschuss.

- 1W4 Pferde des Dorfes werden lahm oder getötet.
- Eine Reihe von Einheimischen wird verletzt. Markiere bei der Siedlung Geschwächt (Nachteil auf Truppen einsetzen, Truppen aufbieten oder Gemeinsam anpacken).
- Der SL wählt einen bei der Jagd anwesenden NSC. Er wird getötet.
- Das Hügelvolk fühlt sich auf irgendeine Weise beleidigt.
- Die Herde ist geschwächt. Wenn du im nächsten Jahr jagst, wird sie vollständig ausgelöscht.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "add \"Aurochs hunting (meat, hide, horn)\" to the Resources list"
- `effects/1/text` — "lead the aurochs hunt, rolling +Defenses"
- `effects/0/listEntry/text` — "Aurochs hunting (meat, hide, horn)"

### expanded-trades

#### `choices/12/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Wohlstand um 1. Wenn du **_die Voraussetzungen nicht mehr erfüllst_**, verringere Wohlstand um 1.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Prosperity by 1 (and decrease it by 1 if you cease to meet the requirements)"

### golden-sapling

#### `choices/4/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, füge „Goldener Setzling“ zur Ressourcenliste hinzu und markiere automatisch die Verbesserung „Reichere Ernte“.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "add \"Golden Sapling\" to the Resources list"
- `effects/1/text` — "automatically mark the Greater Harvest improvement"
- `effects/2/text` — "it generates +1 Surplus"
- `effects/2/when/phrase` — "when **_the steading generates Surplus, even just 1_**"
- `effects/0/listEntry/text` — "Golden Sapling"

#### `choices/5/text`

This row was restructured; the key is gone. Your German:

```
Von nun an erzeugt die Siedlung jedes Mal, wenn sie Überschuss erwirtschaftet, +1 Überschuss (selbst wenn es nur 1 Überschuss ist).
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "add \"Golden Sapling\" to the Resources list"
- `effects/1/text` — "automatically mark the Greater Harvest improvement"
- `effects/2/text` — "it generates +1 Surplus"
- `effects/2/when/phrase` — "when **_the steading generates Surplus, even just 1_**"
- `effects/0/listEntry/text` — "Golden Sapling"

### great-wood-timber

#### `choices/5/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_alle Voraussetzungen erfüllt hast_**, erhöhe Schicksal um 1 und füge „Holz aus dem Großen Wald“ zur Ressourcenliste hinzu.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Timber from the Great Wood\" to the Resources list"
- `effects/2/text` — "the steading has a ready supply of timber for improvements and other projects"
- `effects/3/text` — "the steading consumes 1 less Surplus"
- `effects/3/when/phrase` — "**_every winter_**"
- `effects/1/listEntry/text` — "Timber from the Great Wood"

#### `choices/6/text`

This row was restructured; the key is gone. Your German:

```
Von nun an verfügt die Siedlung über eine zuverlässige Versorgung mit Bauholz für Verbesserungen und andere Projekte und verbraucht in jedem Winter 1 Überschuss weniger.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Timber from the Great Wood\" to the Resources list"
- `effects/2/text` — "the steading has a ready supply of timber for improvements and other projects"
- `effects/3/text` — "the steading consumes 1 less Surplus"
- `effects/3/when/phrase` — "**_every winter_**"
- `effects/1/listEntry/text` — "Timber from the Great Wood"

### greater-harvest

#### `choices/4/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**,, erhöhe Schicksal um 1. Sobald **_die Herbsternte abgeschlossen ist_**, erzeugt die Siedlung +1W4 Überschuss.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "gain +1d4 Surplus"
- `effects/1/when/phrase` — "when **_the autumn harvest is complete_**"

### harnessing-the-stream

#### `choices/reservoir/text`

The English changed after this was translated.

Was:

```
A reservoir for the Stream to pool in, and some way for water to flow uphill
```

Now:

```
A reservoir for the Stream to pool in
```

Your German:

```
Ein Reservoir, in dem sich das Wasser des Bachs sammeln kann, sowie eine Möglichkeit, das Wasser bergauf fließen zu lassen
```

#### `choices/4/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, füge die entsprechende Anlage der Ressourcenliste hinzu und erhöhe Schicksal um 1.

Von nun an gilt: Wenn **_der Frühling anbricht und du mit Schicksal eine 7+ würfelst_**, erzeugt die Siedlung 1 Überschuss.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add them to the Resources list"
- `effects/2/text` — "the steading generates 1 Surplus"
- `effects/2/when/phrase` — "when **_spring breaks forth and you roll a 7+ with Fortunes_**"
- `effects/1/listEntry/text` — "Harnessing the Stream"
- `choices/water-uphill/text` — "Some way for water to flow uphill"

### herd-of-horses

#### `choices/10/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Schicksal um 1 und ersetze „Ein Paar kräftige Zugpferde“ auf der Liste der Besitztümer durch „Eine Pferdeherde“. Notiere dir ihre Größe. Von nun an gilt:

Wenn du die **_Pferde einsetzt, um gemeinsam anzupacken_**, dauert es nur noch halb so lange und kostet nur noch halb so viel. Wenn du **_höchstens die Hälfte der Herde anforderst_**, behandle eine 6- als 7–9.

Wenn **_der Sommer beginnt_**, werden alle Jährlinge zu Pferden (Wert 3, sobald sie ausgebildet sind), alle Fohlen zu Jährlingen (Wert 2), und die Herde bekommt Fohlen (Wert 1) in einer Anzahl von 1W4 + Schicksal (min. 0).

Wenn **_der Winter das Land fest im Griff hat_**, verbraucht die Herde 1 Überschuss je 6 ausgewachsene Pferde oder Jährlinge. Für jeden Überschuss, der nicht verbraucht wird, gehen 1W6 Pferde verloren.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "replace \"a pair of sturdy draft horses\" with \"a herd of horses\" on the Assets list. Make a note of its size"
- `effects/2/text` — "it takes half as long and costs half as much"
- `effects/3/text` — "treat a 6- as a 7-9"
- `effects/4/text` — "any yearlings become horses (Value 3 once trained), any foals become yearlings (Value 2), and the herd gains foals (Value 1) equal to 1d4+Fortunes (min 0)"
- `effects/5/text` — "the herd consumes 1 Surplus per 6 grown or yearling horses; for every Surplus not consumed, 1d6 horses are lost"
- `effects/2/when/phrase` — "when **_you leverage the horses to Pull Together_**"
- `effects/3/when/phrase` — "when **_you Requisition half the herd or less_**"
- `effects/4/when/phrase` — "when **_the seasons change to summer_**"
- `effects/5/when/phrase` — "when **_winter grips the land_**"

### heroic-reputation

#### `choices/8/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhältst du den folgenden Spielzug:

Wenn du zum ersten Mal jemandem von außerhalb Stonetops begegnest, würfle +Schicksal. **Bei einer 10+** sagst du, was sie über dich oder Stonetop gehört hat, und du erhältst einen Vorteil auf deinen nächsten Spielzug gegen sie. **Bei 7–9** sagst du, was sie gehört hat. **Bei einer 6-** entscheidet der SL, was sie gehört hat.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "gain the move: when you first meet someone from beyond Stonetop, roll +Fortunes"

### inn

#### `choices/7/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Schicksal um 1. Gib dem Gasthaus einen Namen füge es zur Ressourcenliste hinzu und zeichne es auf der Karte ein.

Von nun an gilt: Wenn die Jahreszeit wechselt, würfelt die Person, die am freundlichsten ist, +Schicksal: **Bei einer 10+** stelle dem SL drei Fragen über die weite Welt. **Bei 7–9** stelle eine Frage. **Bei einer 6-** stelle eine Frage, aber der SL beschreibt Schwierigkeiten, die auf das Gasthaus oder seine Gäste zurückzuführen sind.

Einmal pro Jahreszeit, wenn du **_1 Überschuss ausgibst und die Leute im Gasthaus zusammenbringst_** (um zu reden, zu feiern, sich zu erholen), streiche eine Beeinträchtigung der Siedlung.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "name the inn, add it to both the Resources list and map"
- `effects/2/text` — "whoever is friendliest rolls +Fortunes for news of the wider world"
- `effects/3/text` — "clear one of the steading's debilities"
- `effects/3/when/phrase` — "once per season, when **_you expend 1 Surplus and bring folks together at the inn_**"
- `effects/1/listEntry/text` — "The inn"

### market

#### `choices/8/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe den Wohlstand um 1. Wenn du **_die Voraussetzungen nicht mehr erfüllst_**, verringere den Wohlstand um 1.

Wenn die Jahreszeiten zu Frühling, Sommer oder Herbst wechseln, der Marktplatz aktiv ist und die Bevölkerung mindestens +1 beträgt, erzeugt der Marktplatz 1 Überschuss.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Prosperity by 1 (and decrease it by 1 if you cease to meet the requirements)"
- `effects/1/text` — "the market generates 1 Surplus"
- `effects/1/when/phrase` — "when **_the seasons change to spring, summer, or autumn, and the market is active, and Population is +1 or better_**"

### mill

#### `choices/7/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Schicksal um 1, füge „Mühle“ zur Ressourcenliste hinzu und zeichne sie auf der Karte ein.

Von nun an erzeugt die Siedlung nach Abschluss der Herbsternte +1 Überschuss. Außerdem haben alle Vorräte eine zusätzliche Anwendung, wenn du dich in Stonetop ausrüstest oder wenn du hast, was du brauchst.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Mill\" to the Resources list and draw it on the map"
- `effects/2/text` — "the steading generates +1 Surplus"
- `effects/3/text` — "each of supplies has 1 extra use"
- `effects/2/when/phrase` — "when **_the autumn harvest is complete_**"
- `effects/3/when/phrase` — "when **_you Outfit from Stonetop or Have What You Need after doing so_**"
- `effects/1/listEntry/text` — "Mill"

### palisade

#### `choices/6/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, steigt Schicksal um 1. Füge „Palisade“ zur Liste der Befestigungen hinzu und zeichne sie auf der Karte ein.

Von nun an hast du einen Vorteil auf Aufgebot, wenn du die Palisade zu deinem Vorteil nutzt.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Palisade\" to the Fortifications list and draw it on the map"
- `effects/2/text` — "you have advantage to Deploy"
- `effects/2/when/phrase` — "when **_you take advantage of the palisade_**"
- `effects/1/listEntry/text` — "Palisade"

### permanent-logging-camp

#### `choices/extra-wagon/text`

The English changed after this was translated.

Was:

```
An extra wagon (Value 3) and extra horse or mule (Value 3) to
```

Now:

```
An extra wagon (Value 3) and extra horse or mule (Value 3) to haul timber to and from Stonetop
```

Your German:

```
Ein zusätzlicher Wagen (Wert 3) und ein zusätzliches Pferd oder Maultier (Wert 3), um Holz nach Stonetop und zurück zu transportieren
```

#### `choices/8/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_alle Voraussetzungen erfüllst_**, füge „Abholzung: Bauholz, Holz“ zur Ressourcenliste hinzu und erhöhe Schicksal um 1.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Logging: timber, wood\" to the Resources list"
- `effects/2/text` — "the steading consumes 1 less Surplus than usual"
- `effects/3/text` — "the logging camp consumes 1 Surplus, or else it ceases operation"
- `effects/2/when/phrase` — "**_in winter, as long as the camp is in operation_**"
- `effects/3/when/phrase` — "**_every summer_**"
- `effects/1/listEntry/text` — "Logging: timber, wood"

#### `choices/9/text`

This row was restructured; the key is gone. Your German:

```
Solange das Holzfällerlager in Betrieb ist, verfügt die Siedlung fortan über einen ständigen Vorrat an Bauholz für Verbesserungen und andere Projekte und verbraucht im Winter 1 Überschuss weniger als üblich. Allerdings verbraucht das Holzfällerlager jeden Sommer 1 Überschuss, andernfalls stellt es den Betrieb ein.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Logging: timber, wood\" to the Resources list"
- `effects/2/text` — "the steading consumes 1 less Surplus than usual"
- `effects/3/text` — "the logging camp consumes 1 Surplus, or else it ceases operation"
- `effects/2/when/phrase` — "**_in winter, as long as the camp is in operation_**"
- `effects/3/when/phrase` — "**_every summer_**"
- `effects/1/listEntry/text` — "Logging: timber, wood"

### raincatching

#### `choices/5/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, erhöhe Schicksal um 1 und füge „Regenauffangsystem“ zur Ressourcenliste hinzu.

Von nun an gilt: Wenn **_der Sommer kommt und du mit Schicksal eine 7+ würfelst_**, erzeugt die Siedlung 1 Überschuss.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Raincatching\" to the Resources list"
- `effects/2/text` — "the steading generates 1 Surplus"
- `effects/2/when/phrase` — "when **_summer comes and you roll a 7+ with Fortunes_**"
- `effects/1/listEntry/text` — "Raincatching"

### roadbuilding

#### `choices/6/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_alle Voraussetzungen erfüllst_**, kannst du die Straßen der Schöpfer ausbauen oder reparieren. Für jeweils ca. 25 Meilen Straße erfordert dies gemeinsam anzupacken, eine Jahreszeit Arbeit, 2 Überschuss zur Versorgung der Arbeiter sowie ein Beutel voller Silbermünzen für Löhne und Material.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "you can expand or repair the Makers' Roads. Every ~25 miles of road requires Pulling Together, one season of labor, 2 Surplus to feed the laborers, and a purse of silvers for payment and supplies"

### standing-watch

#### `choices/5/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, füge „Stehende Wache“ zur Liste der Befestigungen hinzu.

Zu Beginn jeder Jahreszeit verbraucht die Wache 1 Überschuss – andernfalls löst sie sich auf.

Wenn du **_die Wache gezielt in einem Spielzug einbeziehst_**, behandle Verteidigung als 1 höher, als sie tatsächlich ist.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "add \"standing watch\" to the Fortifications list"
- `effects/1/text` — "the watch consumes 1 Surplus, or it disbands"
- `effects/2/text` — "treat Defenses as 1 higher than they are"
- `effects/1/when/phrase` — "at **_the start of each season_**"
- `effects/2/when/phrase` — "when **_you specifically involve the watch in a move_**"
- `effects/0/listEntry/text` — "Standing watch"

### stone-wall

#### `choices/6/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllst_**, füge „Steinmauer“ zur Liste der Befestigungen hinzu (streiche „Palisade“, falls du sie hast) und zeichne sie auf der Karte ein. Von nun an gilt:

Wenn du **_die Steinmauer zu deinem Vorteil nutzt_**, hast du beim Einsetzen einen Vorteil.

Wenn **_der Winter das Land fest im Griff hat_**, verbraucht die Siedlung 1 Überschuss weniger als gewöhnlich.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "add \"Stone Wall\" to the Fortifications list (erase \"Palisade\" if you had it) and draw it on the map"
- `effects/1/text` — "you have advantage to Deploy"
- `effects/2/text` — "the steading consumes 1 less Surplus than normal"
- `effects/1/when/phrase` — "when **_you take advantage of the stone wall_**"
- `effects/2/when/phrase` — "when **_winter grips the land_**"
- `effects/0/listEntry/text` — "Stone Wall"

### township

#### `choices/7/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_alle Voraussetzungen erfüllst_**, ändere Größe in Stadt und die Bevölkerung auf +0. Von nun an gilt:

Wenn du **_Truppen aufbietest, Gemeinsam anpackst oder handelst & tauschst_**, hast du einen Vorteil.

Wenn **_der Frühling oder Sommer beginnt_**, erzeugt die Stadt Überschuss in Höhe von Bevölkerung +1.

Wenn jedoch **_der Winter das Land fest im Griff hat_**, würfle 2W6 + Bevölkerung, um Überschuss zu verbrauchen, statt 1W4 + Bevölkerung.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "change Size to town"
- `effects/1/text` — "change Population to +0"
- `effects/2/text` — "you have advantage"
- `effects/3/text` — "the town generates Surplus equal to Population+1"
- `effects/4/text` — "roll 2d6+Population to consume Surplus, instead of 1d4+Population"
- `effects/2/when/phrase` — "when **_you Muster, Pull Together, or Trade & Barter_**"
- `effects/3/when/phrase` — "when **_the seasons change to spring or summer_**"
- `effects/4/when/phrase` — "when **_winter grips the land_**"

### trade-with-barrier-pass

#### `choices/8/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_alle Voraussetzungen erfüllst_**, erhöhe Schicksal um 1 und füge „Handel mit dem Barrierenpass (Holz, Elfenbein, Pergament, feine Wolle, Ziegen, Schafe)“ hinzu.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Trade with Barrier Pass (timber, ivory, parchment, fine wool, goats, sheep)\""
- `effects/2/text` — "Stonetop gains +1 Surplus from trade"
- `effects/3/text` — "you have advantage"
- `effects/2/when/phrase` — "when **_spring bursts forth and Stonetop has at least 1 Surplus_**"
- `effects/3/when/phrase` — "when **_you Trade & Barter for timber, ivory, fine wool, goats, sheep, parchment, vellum, or ink_**"
- `effects/1/listEntry/text` — "Trade with Barrier Pass (timber, ivory, parchment, fine wool, goats, sheep)"

#### `choices/9/text`

This row was restructured; the key is gone. Your German:

```
Von nun an gilt: Wenn **_der Frühling anbricht und Stonetop mindestens 1 Überschuss besitzt_**, erhält Stonetop durch den Handel +1 Überschuss. Außerdem hast du einen Vorteil, wenn du mit Holz, Elfenbein, feiner Wolle, Ziegen, Schafen, Pergament, Velinpapier oder Tinte handelst & tauschst.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Fortunes by 1"
- `effects/1/text` — "add \"Trade with Barrier Pass (timber, ivory, parchment, fine wool, goats, sheep)\""
- `effects/2/text` — "Stonetop gains +1 Surplus from trade"
- `effects/3/text` — "you have advantage"
- `effects/2/when/phrase` — "when **_spring bursts forth and Stonetop has at least 1 Surplus_**"
- `effects/3/when/phrase` — "when **_you Trade & Barter for timber, ivory, fine wool, goats, sheep, parchment, vellum, or ink_**"
- `effects/1/listEntry/text` — "Trade with Barrier Pass (timber, ivory, parchment, fine wool, goats, sheep)"

### weapons-of-war

#### `choices/10/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Voraussetzungen erfüllt_**, erhöhte die Verteidigung um 1 und füge „Kriegswaffen“ zur Liste der Befestigungen hinzu. **_In jedem Frühjahr_** muss das Dorf 1 Überschuss aufwenden, um die Waffen der Siedlung instand zu halten und zu ersetzen.

Von nun an gilt: Wenn du **_dich in Stonetop ausrüstest oder wenn du hast, was du brauchst_**, kannst du Streitkolben, Dreschflegel, Streitäxte, Kriegshämmer und alle Arten von Schwertern wie gewöhnliche Gegenstände behandeln, als wären sie bereits auf der Ausrüstungsbeilage aufgeführt. Streitäxte und Schwerter haben „Durchdringung x“, wobei x dem aktuellen Wohlstand der Siedlung entspricht.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "increase Defenses by 1"
- `effects/1/text` — "add \"Weapons of War\" to the Fortifications list"
- `effects/2/text` — "the village must expend 1 Surplus to maintain and replace the town's weapons"
- `effects/3/text` — "you can treat maces, flails, battleaxes, warhammers, and all types of swords as common items"
- `effects/4/text` — "battleaxes and swords have \"x piercing,\" where x is the steading's current Prosperity"
- `effects/2/when/phrase` — "**_each spring_**"
- `effects/3/when/phrase` — "when **_you Outfit from Stonetop or Have What You Need after doing so_**"
- `effects/1/listEntry/text` — "Weapons of War"

### well-trained-militia

#### `choices/9/text`

This row was restructured; the key is gone. Your German:

```
Wenn du **_die Miliz einsetzt, und sie wendet dabei eine der von ihr erlernten Taktiken an_**, handelt ihr wahrscheinlich aus einer Position der Stärke heraus (bei 7–9 wählst du die Konsequenz, nicht der SL).

Wenn **_die Miliz zwei oder mehr Taktiken erlernt hat_**, erhöhe die Verteidigung um 1.

**_In jedem Sommer_** muss die Miliz 1 Überschuss aufwenden und einige Wochen trainieren, ansonsten verliert sie ihre Ausbildung in einer Taktik.
```

Untranslated keys left in this document — the slots these words belong in:

- `effects/0/text` — "you are likely acting from a position of strength (you pick the consequence on a 7-9, not the GM)"
- `effects/1/text` — "when the militia has trained in 2+ tactics, increase Defenses by 1"
- `effects/2/text` — "the militia must spend 1 Surplus and a week or so practicing, or else lose its training in 1 tactic"
- `effects/0/when/phrase` — "when **_you Deploy using one of the militia's trained tactics_**"
- `effects/2/when/phrase` — "**_each summer_**"

## steadfasts

### stonetop

#### `neighborPlaces/other/subtitle`

The English changed after this was translated.

Was:

```
Barrier Pass, the Manmarch, etc.
```

Now:

```
The Manmarch, etc.
```

Your German:

```
Barrierenpass, die Grenzmark, usw.
```

#### `assets/items/a-pair-of-hardy-draft-horses-followers-large-powerful-keen-nosed-hardy-hp-10-each-damage-d6-3-hand-close-forceful-instinct-to-panic-cost-care-grooming`

This row was restructured; the key is gone. Your German:

```
Ein Paar kräftige Zugpferde, Gefolgsleute (Groß, Kräftig, Guter Geruchssinn, Widerstandsfähig): Jeweils 10 TP, Schaden 1W6+3 (Hand, Kurz, Wuchtig), Instinkt: in Panik geraten, Kosten: Betreuung & Pflege.
```

Untranslated keys left in this document — the slots these words belong in:

- `impressions/0/text` — "Drip drip drip of snow melting from roofs, collecting in jugs and puddles"
- `impressions/1/text` — "Petrichor smell on a southerly breeze"
- `impressions/2/text` — "Hopeful green poking through deadbrown grass and soil"
- `impressions/3/text` — "Bare skin reveling in the still-chilly sun; cheerful voices; smiles and songs"
- `impressions/4/text` — "Blinding sun outside, cave-like dark inside"
- `impressions/5/text` — "Crops swaying and rippling in the breeze"
- `impressions/6/text` — "Fireflies like galaxies over the fields at dusk"
- `impressions/7/text` — "Dark storm clouds looming in the south, rumbling, flashing"
- `impressions/8/text` — "CRACKOOM of thunder, blinding flash as lightning touches the Stone"
- `impressions/9/text` — "Crops bobbing heavy with seed, the Great Wood aflame with color"
- `impressions/10/text` — "Work-song, shouts and laughter, grunts of effort"
- `impressions/11/text` — "Rosy cheeks, breath visible in the morning air"
- …and more; see `steadfasts.json`.
