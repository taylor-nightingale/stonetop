// Inline "Draw" buttons for journal prose. A `@DrawTable[<uuid>]{<label>}` token next to a dice
// table is rendered as a roll button, plus a GM-only delegated click handler that draws from the
...
//
// This is complementary infrastructure to applyJournalRollTables (which adds a die to the "Roll"
// header of the static <table>s the gazetteer bakes in): different inputs (a prose token referencing
// a real RollTable document vs. an inline <table>), different class (.stonetop-draw-table vs.
// .stonetop-roll-table-btn). They coexist. The token form is what makes the standalone Wonder Tables
// pack usable from any journal/prose.
...
const _l = (key, fallback) => game.i18n?.localize?.(key) ?? fallback;
...
export async function onClickDrawTable(event) {
	...
	if (!game.user?.isGM) {
		ui.notifications?.warn(_l("stonetop.journal.drawTable.gmOnly", "Only the GM can draw from this table."));
		return;
	}
	const table = await fromUuid(a.dataset.uuid);
	if (table?.draw) return table.draw();
	ui.notifications?.warn(_l("stonetop.journal.drawTable.notFound", "Table not found."));
}
