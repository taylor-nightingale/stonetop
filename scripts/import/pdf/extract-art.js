...
import { extractSteadingArt, extractSeasonsArt, extractArchBadges } from "./steading-art.js";
...
const STEADING_ASSET_DIR = "assets/content/steading"; // committed: the Fortunes/Surplus arch panels
if (existsSync(BOOK_I)) {
	...
	// The Fortunes/Surplus arch panels — rendered and cropped, not pulled: the playbook page is one
	// mask. Trade dress, like the season glyphs, so they go to the COMMITTED assets directory: the
	// sheet's two crowned ratings should not depend on the reader having installed anything.
	const ab = extractArchBadges(BOOK_I, STEADING_ASSET_DIR);
	console.log(`extracted ${ab.written.length} arch badge(s) -> ${STEADING_ASSET_DIR}/` + (ab.missing.length ? `  (missing: ${ab.missing.join(", ")})` : ""));
} else {
