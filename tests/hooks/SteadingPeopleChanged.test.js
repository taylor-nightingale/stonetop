...
describe("onPreUpdateSteadingPeople", () => {
	it("stashes the delta on the update options", () => {
		const options = {};
		onPreUpdateSteadingPeople(steading(), { system: { folk: [{ id: "p1", name: "Willa Fletcher" }] } }, options);
		expect(options[PEOPLE_DELTA_KEY]).toEqual({ people: ["p1"] });
	});
	...
	it("ignores actors that are not steadings", () => {
		const options = {};
		onPreUpdateSteadingPeople({ type: "character", system: {} }, { system: { folk: [{ id: "p1", name: "X" }] } }, options);
		expect(options[PEOPLE_DELTA_KEY]).toBeUndefined();
	});
});
...
describe("onUpdateSteadingPeople", () => {
	it("syncs the changed rows on the active GM's client", async () => {
		...
		await onUpdateSteadingPeople(steading(sync), {}, { [PEOPLE_DELTA_KEY]: { people: ["p1"] } });
		expect(sync).toHaveBeenCalledOnce();
		expect(sync.mock.calls[0][0].people).toEqual(["p1"]);
	});
