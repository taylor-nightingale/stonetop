...
// The repo returns Follower-entity-shaped objects: the tag VALUE is exposed as `tags` and the stat
// block's printed choices as `tagOptions`; instinct/cost are still stored { selected, options }
// objects (see model/data/character/Follower.js).
const packTulpa = {
	slug: "tulpa", name: "Tulpa", img: "systems/stonetop/assets/content/wonders/markers/marker-triskele.png",
	tags:     ["Spirit"], tagOptions: ["Spirit", "eager", "fierce"],
...
describe("migrateArcanaFollowerPackData", () => {
	...
	it("refreshes the authored stat block: options, selectable-move choice group, and marker icon", async () => {
		...
		expect(u.system.tagList).toEqual(["Spirit"]);
		expect(u.system.tagOptions).toEqual(["Spirit", "eager", "fierce"]);
